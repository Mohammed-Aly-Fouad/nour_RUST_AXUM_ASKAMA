...
INSERT INTO public.categories (name, name_ar, parent_id, notes) VALUES 
...
('Notebooks, Notepads & Journals', 'الكشاكيل والدفاتر والنوت بوك', 
    (SELECT id FROM public.categories WHERE name = 'Notebooks' AND parent_id IS NULL), 
...
('Pencils & Lead Refills', 'أقلام رصاص وسنون ', 
    (SELECT id FROM public.categories WHERE name = 'Writing & Correction' AND parent_id IS NULL), 
...
('Adhesives & Glues', 'مواد وأدوات اللصق', 
    (SELECT id FROM public.categories WHERE name = 'Office Supplies' AND parent_id IS NULL), 
...
('Files, Folders & Envelopes', 'ملفات ودوسيهات وأظرف ورقية', 
    (SELECT id FROM public.categories WHERE name = 'Filing & Organization' AND parent_id IS NULL), 
...
('Calculators, Storage & Tech Accessories', 'الآلات الحاسبة ووسائط التخزين وإكسسوارات التكنولوجيا', 
    (SELECT id FROM public.categories WHERE name = 'Technology & Electronics' AND parent_id IS NULL), 
...
('Gift Wrapping Supplies', 'مستلزمات وتغليف الهدايا', 
    (SELECT id FROM public.categories WHERE name = 'Toys, Games, Gifts & Party Supplies' AND parent_id IS NULL), 
...
('Geometry & Measuring Tools', 'أدوات القياس والهندسة', 
    (SELECT id FROM public.categories WHERE name = 'Drawing and measuring tools' AND parent_id IS NULL), 
...
('Pencil Cases & School Accessories', 'المقالم والمستلزمات المدرسية', 
    (SELECT id FROM public.categories WHERE name = 'School & College Supplies' AND parent_id IS NULL), 
